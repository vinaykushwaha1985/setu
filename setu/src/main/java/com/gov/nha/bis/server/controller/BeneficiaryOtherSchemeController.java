...
import com.gov.nha.bis.server.rest.service.BeneficiaryOtherDataService;
@Controller
public class BeneficiaryOtherSchemeController {
	...
	@RequestMapping(value="/beneficiarySearchOther",method = RequestMethod.POST)
			...
			Locale locale,Model model) throws IOException
	{
		...
		List<SchemeOther> villageList= null;
		...
		if(applicationForm.getRuralUrbanFlag().equalsIgnoreCase("R")){
			...
			villageList= BeneficiaryOtherDataService.getRuralVillList(applicationForm.getStateCodeRural(),applicationForm.getDistrictCodeRural(),
					applicationForm.getRuralUrbanFlag(),applicationForm.getSchemeid(),applicationForm.getFamilyidScheme(),
					applicationConstantConfig.BEN_OTHER_SCHEME_SEARCH_URL);
			...
			//if(CheckRedisEnbleFunction.convertArrayToList(applicationConstantConfig.SEARCH_SERVICE_REDIS_ENABLE_LIST.split(",")).contains(applicationForm.getStateCodeUrban())) {
				villageList= BeneficiaryOtherDataService.getRuralVillList(applicationForm.getStateCodeUrban(),applicationForm.getDistrictCodeUrban(),"",
						applicationForm.getSchemeid(),applicationForm.getFamilyidScheme(),applicationConstantConfig.BEN_OTHER_SCHEME_SEARCH_URL);
	...
	appHistoryService.saveEventDetails(Integer.valueOf(stateCode), districtCode,applicationForm.getRuralUrbanFlag(),userId,
			userId,userType,roleId,"beneficiarySearchOther",ip,name);
