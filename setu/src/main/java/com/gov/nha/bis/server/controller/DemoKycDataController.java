...
@Controller
public class DemoKycDataController {



	private static final Logger logger = LogManager.getLogger(DemoKycDataController.class);
	...
	@RequestMapping(value="/demoKycData",method = RequestMethod.POST)
	public @ResponseBody String  demoKycData(HttpServletRequest request,
		...
		try {
			...
			if(!ObjectUtils.isEmpty(verifyStaty)) {
				...
				if(verifyStatyJson.getJSONObject(0).getString("status").equalsIgnoreCase("Y")) {
					...
					String updateUidTokenRes=	beneficiaryCreateCardService.updateDemoKcyUidToken(applicationForm.getGuid(),applicationForm.getState_code(),applicationForm.getUid_token());
					logger.info("createCardResponse==="+updateUidTokenRes);
					if(!ObjectUtils.isEmpty(updateUidTokenRes)) {
						JSONObject upRes= new JSONObject(updateUidTokenRes);
						if(!ObjectUtils.isEmpty(upRes.getString("status"))) {
							if(upRes.getString("status").equalsIgnoreCase("Y")) {
								saveMap.put("status", "Y");
								saveMap.put("refId", applicationForm.getGuid());
							}else {
					...
					String	response=saveKycDataService.saveKyc(applicationForm.getRefernceid(),
							applicationForm.getName_s(), 
							applicationForm.getFather_name_s().substring(0,3),
							applicationForm.getFather_name_s().substring(4),
							applicationForm.getGender_s(),applicationForm.getDob_s(),applicationForm.getAddress_s(),
							applicationForm.getState_name_s(),	applicationForm.getDistrict_name_s(), applicationForm.getUid_auth_type(),
							...
							applicationForm.getVtcCode(),applicationForm.getWardCode(),"",2);
					...
					if(!ObjectUtils.isEmpty(response)) {
						...
						if(demoRes.getBoolean("status")) {
							 ...
							 * applicationForm.getState_code(),
							 * "49",applicationForm.getRuralUrbanFlag(),userId);
							 * 
							 * 
							 * if(beneficiarySeccForm.getRuralUrbanFlag().equalsIgnoreCase("R")) {
							 * 
							 * beneficiaryUpdateStatusService.beneficiaryUpdateStatus(applicationForm.
							 * getRefernceid(),
							 * applicationForm.getState_code(),beneficiarySeccForm.getDistrictCodeRural(),
							 * beneficiarySeccForm.getBlockCodeRural(),
							 * beneficiarySeccForm.getVillageCodeRural(),"","","49",
							 * applicationForm.getRuralUrbanFlag(),userId);
							 * 
							 ...
							 * beneficiarySeccForm.getWardCodeUrban(),"","","49",
							 * applicationForm.getRuralUrbanFlag(),userId); }
							 */
